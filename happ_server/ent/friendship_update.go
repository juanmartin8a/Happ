...
func (fuo *FriendshipUpdateOne) sqlSave(ctx context.Context) (_node *Friendship, err error) {
	...
	if id, ok := fuo.mutation.UserID(); !ok {
		return nil, &ValidationError{Name: "user_id", err: errors.New(`ent: missing "Friendship.user_id" for update`)}
	} else {
		_spec.Node.CompositeID[0].Value = id
	}
	if id, ok := fuo.mutation.FriendID(); !ok {
		return nil, &ValidationError{Name: "friend_id", err: errors.New(`ent: missing "Friendship.friend_id" for update`)}
	} else {
