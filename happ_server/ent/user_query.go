...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFollow; query != nil {
		if err := uq.loadFollow(ctx, query, nodes,
			func(n *User) { n.Edges.Follow = []*Follow{} },
			func(n *User, e *Follow) { n.Edges.Follow = append(n.Edges.Follow, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadFollow(ctx context.Context, query *FollowQuery, nodes []*User, init func(*User), assign func(*User, *Follow)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
