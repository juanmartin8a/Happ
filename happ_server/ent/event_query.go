...
type EventQuery struct {
	...
	withNamedEventUsers                 map[string]*EventUserQuery
...
func (eq *EventQuery) QueryEventReminderNotifications() *EventReminderNotificationQuery {
	query := (&EventReminderNotificationClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryEventUsers chains the current query on the "event_users" edge.
func (eq *EventQuery) QueryEventUsers() *EventUserQuery {
	query := (&EventUserClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withEventReminderNotifications: eq.withEventReminderNotifications.Clone(),
		withEventUsers:                 eq.withEventUsers.Clone(),
...
func (eq *EventQuery) WithEventReminderNotifications(opts ...func(*EventReminderNotificationQuery)) *EventQuery {
	query := (&EventReminderNotificationClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withEventReminderNotifications = query
	return eq
}

// WithEventUsers tells the query-builder to eager-load the nodes that are connected to
// the "event_users" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithEventUsers(opts ...func(*EventUserQuery)) *EventQuery {
	query := (&EventUserClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withEventUsers = query
	return eq
}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	var (
		...
		loadedTypes = [3]bool{
			eq.withUsers != nil,
			eq.withEventReminderNotifications != nil,
			eq.withEventUsers != nil,
		}
	)
	...
	if query := eq.withEventReminderNotifications; query != nil {
		if err := eq.loadEventReminderNotifications(ctx, query, nodes,
			func(n *Event) { n.Edges.EventReminderNotifications = []*EventReminderNotification{} },
			func(n *Event, e *EventReminderNotification) {
				n.Edges.EventReminderNotifications = append(n.Edges.EventReminderNotifications, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := eq.withEventUsers; query != nil {
		if err := eq.loadEventUsers(ctx, query, nodes,
			func(n *Event) { n.Edges.EventUsers = []*EventUser{} },
			func(n *Event, e *EventUser) { n.Edges.EventUsers = append(n.Edges.EventUsers, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range eq.withNamedEventReminderNotifications {
		if err := eq.loadEventReminderNotifications(ctx, query, nodes,
			func(n *Event) { n.appendNamedEventReminderNotifications(name) },
			func(n *Event, e *EventReminderNotification) { n.appendNamedEventReminderNotifications(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range eq.withNamedEventUsers {
		if err := eq.loadEventUsers(ctx, query, nodes,
			func(n *Event) { n.appendNamedEventUsers(name) },
			func(n *Event, e *EventUser) { n.appendNamedEventUsers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (eq *EventQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Event, init func(*Event), assign func(*Event, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(event.UsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(event.UsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(event.UsersPrimaryKey[0]))
...
func (eq *EventQuery) loadEventReminderNotifications(ctx context.Context, query *EventReminderNotificationQuery, nodes []*Event, init func(*Event), assign func(*Event, *EventReminderNotification)) error {
...
func (eq *EventQuery) loadEventUsers(ctx context.Context, query *EventUserQuery, nodes []*Event, init func(*Event), assign func(*Event, *EventUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Event)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithNamedEventReminderNotifications(name string, opts ...func(*EventReminderNotificationQuery)) *EventQuery {
	query := (&EventReminderNotificationClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if eq.withNamedEventReminderNotifications == nil {
		eq.withNamedEventReminderNotifications = make(map[string]*EventReminderNotificationQuery)
	}
	eq.withNamedEventReminderNotifications[name] = query
	return eq
}

// WithNamedEventUsers tells the query-builder to eager-load the nodes that are connected to the "event_users"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithNamedEventUsers(name string, opts ...func(*EventUserQuery)) *EventQuery {
	query := (&EventUserClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if eq.withNamedEventUsers == nil {
		eq.withNamedEventUsers = make(map[string]*EventUserQuery)
	}
	eq.withNamedEventUsers[name] = query
	return eq
}
